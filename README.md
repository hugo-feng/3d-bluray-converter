...
https://github.com/hugo-feng/3d-bluray-converter/releases/download/v2.6.0/3DBlurayConverter_v2.6.0_portable.zip
...
- **进度与倒计时**：解流 / 编码 / 音频 / 混流各阶段实时进度、帧率；
  同时显示**本阶段剩余时长**与**总剩余时长**（全部阶段合计，随进度自动校准）；
  长时间无输出时自动提示"仍在运行"（大文件写盘 / 解码器初始化较慢时不再像卡死）
- **开始前确认**：点击「开始转换」后弹窗列出**总阶段数**与**各阶段预计耗时**（合并为合计预计），
  确认后才真正开始，可一键取消不做任何改动
...
- **任务中锁定参数**：转换 / 拼接进行中，源文件、输出设置、编码设置、音频、高级等
  全部参数自动锁定（暗灰显示），避免误操作改坏配置
- **选项悬停说明**：鼠标悬停「编码器 / 速度 / 质量模式 / 质量档」即显示各选项的区别与推荐建议
